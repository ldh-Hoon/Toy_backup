...
try:
    response = requests.get(url+f"/register/{name}/{pw}")
    data = response.text

    response = requests.get(url+f"/login/{name}/{pw}")
...
async def live_call():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url+"/live") as response:
...
async def get_message():
    async with aiohttp.ClientSession() as session:
        async with session.get(url+f"/get/{name}") as response:
            if response.status == 200: # 요청 성공
                text = await response.text()
                data = text
...
async def send(text):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url+f"/chat/{name}/{text}") as response:
            if response.status == 200: # 요청 성공
                text = await response.text()
                data = text
            else: # 요청 실패
...
send_button = tkinter.Button(window, text="전송은 엔터", command=send_message(""))
